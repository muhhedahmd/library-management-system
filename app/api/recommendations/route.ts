...
export async function GET(req: NextRequest) {
  try {
    ...
    switch (method) {
      case "rating":
        recommendations = await getRatingBasedRecommendations(userId, limit)
        break
      case "favorite":
...
async function getRatingBasedRecommendations(userId: string, limit: number) {
  // Get books that the user has rated highly (4 or 5)
  const userHighRatings = await prisma.rating.findMany({
    where: {
      userId,
      rating: {
  ...
  const categoryIds = userHighRatings.map((rating) => rating.book.categoryId)
  const authorIds = userHighRatings.map((rating) => rating.book.authorId)
  const ratedBookIds = userHighRatings.map((rating) => rating.bookId)
...
async function getHybridRecommendations(userId: string, limit: number) {
  ...
  const [ratingRecs, favoriteRecs, categoryRecs, authorRecs, popularBooks] = await Promise.all([
    getRatingBasedRecommendations(userId, limit),
