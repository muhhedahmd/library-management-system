"use client";
import { Calendar, Edit, Library, Mail, Phone } from "lucide-react";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const blurProfile = CachedUser?.profile?.profilePictures
    ? CachedUser?.profile?.profilePictures[0]
    : null;
              ...
              <Card className="w-full  md:sticky static top-[4.5rem] md:w-[35%]">
                <CardHeader className="flex flex-col items-center text-center pb-2">
                  <Avatar className="h-24 w-24 mb-4">
                    <AvatarImage
                      src={blurProfile?.secureUrl || ""}
                      alt={CachedUser.name!}
                    />
                    <AvatarFallback>
                      {CachedUser.name!.split(" ")
                        .map((n) => n[0])
                        .join("")}
                    </AvatarFallback>
                  </Avatar>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>
                        Joined{" "}
                        {formatDistanceToNow(new Date(CachedUser?.createdAt), {
                          addSuffix: true,
                        })}
                      </span>
                    </div>
                          ...
                          {CachedUser.profile.website && (
                            <div className="text-sm ">
                              <div className="flex flex-row justify-start items-center gap-2 ">
                                ...
                                {Object.keys(CachedUser.profile.website).map(
                                  (website: string, i: number) => {
                                    const link = CachedUser.profile
                                      .website as Record<string, string>;
                                    return (
                                      <Link
                                        style={{}}
