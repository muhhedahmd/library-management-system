...
export default function PDFReader({ book, readingHistory, pdfUrl }: PDFReaderProps) {
  ...
  async function onDocumentLoadSuccess({ numPages }: PDFDocumentProxy) {
    ...
    setIsLoading(false)
    if (!book.pages && numPages) {
      try {
        const fixPgnumOfBook = await fetch(process.env.NEXT_PUBLIC_API! + `api/books/fixPages?bookId=${book.id}`, {
          method: "PUT",
          body: JSON.stringify({
            pages: numPages
    ...
    } else if (book.pages && +book.pages !== numPages) {
      try {
        const fixPgnumOfBook = await fetch(process.env.NEXT_PUBLIC_API! + `api/books/fixPages?bookId=${book.id}`, {
          method: "PUT",
          body: JSON.stringify({
    ...
    const pagesRead =readingHistory?.pagesRead  &&     readingHistory?.pagesRead || 0
    const bookPages =numPages  &&     numPages || 0
    ...
    useEffect(() => {
// console.log({
//   isequal :pagesRead === bookPages
// })
      // if (readingHistory?.completed  && (pagesRead === bookPages)) return
      if (currentPage !== prevCurrentPage.current || readingTime !== prevReadingTime.current) {
        setNeedsUpdate(true)
        prevCurrentPage.current = currentPage
        prevReadingTime.current = readingTime
      }
    }, [currentPage, readingTime, readingHistory, pagesRead, bookPages])
            ...
            <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <div className="flex  justify-center items-center gap-3">
                        ...
                        <p className="text-sm font-bold">
                          {Math.round(( pagesRead /bookPages) * 100)}%
                        </p>
                      </div>
              ...
              <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                ...
                <SheetContent className=" overflow-auto" side="right">
                  ...
                  <ScrollArea className=" overflow-auto py-4 ">
              ...
              >
                ...
                <Sheet
                  ...
                  <SheetContent side="right">
                    ...
                    <div className="py-4">
                      <div className="space-y-4 overflow-auto h-[98vh] px-6">
