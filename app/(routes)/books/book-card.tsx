...
import { ShoppingCart, Heart, LucideView } from "lucide-react"
...
export function BookCard({ book }: BookCardProps) {
  ...
  const { isLoading: isBookFavLoading, data: isBookFavData } = useGetIsBookFavQuery({
    bookId: book.id,
  })

  // const bookCover = book?.bookCovers?.[0]
  // const hasCover = !!bookCover?.fileUrl
      ...
      <Link href={`/books/${book.id}`} className="flex-1 flex flex-col">
        ...
        <div className="group relative aspect-square w-full overflow-hidden">
          ...
          >
            {book.category.parentId !== null ? (
              <>
                <p
                  className="cursor-pointer rounded-sm hover:underline transition-all duration-200"
                  onClick={() => router.push(`/books?categoryId=${book.category.parentId}`)}
                >
                  {book?.category?.parent?.name && book?.category?.parent?.name?.length > 20
                    ? book?.category?.parent?.name?.slice(0, 20) + "..."
                    : book?.category?.parent?.name}
                </p>
                {" / "}
                <p
                  className="cursor-pointer hover:underline rounded-sm transition-all duration-200"
                  onClick={() => router.push(`/books?categoryId=${book.category.id}`)}
                >
                  {book.category.name.length > 10
                    ? book.category.name.slice(0, 10) + "..."
                    : book.category.name}
                </p>
              </>
            ) : (
              <p
                ...
                onClick={() => router.push(`/books?categoryId=${book.category.id}`)}
              >
                {book.category.name.length > 20
                  ? book.category.name.slice(0, 30) + "..."
                  : book.category.name}
              </p>
            )}
          </Badge>
          ...
          <div
            onClick={(e) => { e.preventDefault(); e.stopPropagation() }}
            className="absolute bottom-0 left-0 w-full transform translate-y-full
              transition-transform duration-300 ease-in-out group-hover:translate-y-0
              flex-row flex justify-center items-start h-fit
        ...
        >
          ...
          {isBookFavLoading || isLoading ? (
            <Button disabled variant="outline" size="icon" className="cursor-default animate-pulse h-9 w-9" />
          ) : (
            <Button
              onClick={() => {
                toggle({ totalFavorites: +book.totalFavorites, bookId: book.id })
                  .catch((e) => console.error("fav toggle failed", e))
              }}
              variant="outline" size="icon" className="h-9 w-9"
            >
              <Heart className={cn("h-4 w-4", isBookFavData && "fill-red-500 stroke-red-500")} />
              {book.totalFavorites}
            </Button>
          )}
        </CardFooter>
